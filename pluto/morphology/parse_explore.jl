...
md"""*Parse a form*: $(@bind s TextField())"""
...
"Format markdown display of parsing results."
function showparses()
	
	hdr = "Parses of $(s):\n"
	mdlines = []
	for parsedform in parselist
		lexu = lexemeurn(parsedform)
		push!(mdlines, "- **$(label(greekForm(parsedform)))**, *from* **$(lemmalabel(lexu, dict = dict))**")
	end
	if isempty(mdlines) 
		Markdown.parse(hdr * "None.")
	else
		hdr * join(mdlines, "\n") |> Markdown.parse
	end
end
...
isnothing(parser) || isempty(s) ? md"" : showparses()
...
# ╔═╡ 3af226b8-d665-42c6-8e9a-49a293b83ba2
tokencount = isnothing(analyzedlexical) ? 0 : length(analyzedlexical)

# ╔═╡ cc162d72-f151-4229-86bc-fed3f4492736
isnothing(analyzedlexical)  ? md"" : md"Parsed corpus with **$(length(corpus.passages))** citable passages and **$(tokencount)** lexical tokens"
