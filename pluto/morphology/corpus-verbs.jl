...
# ╔═╡ cd01671c-3c40-11ee-1ba9-e1acf2bd8767
begin
...
# ╔═╡ 46832eb6-eaef-4459-bf59-3445a808b5ce
...
md"> **Load data from user selections: parser and corpus**"
...
# ╔═╡ f042d16c-fce9-4a24-8525-58cefad27b4f
...
# ╔═╡ d38a5219-63dc-47bb-bf53-d2863f2bf636
# ╠═╡ show_logs = false
...
# ╔═╡ 63bb2ec6-8748-436f-a5e3-376d41624176
"""Given a list of `AnalyzedToken`s, filter out all tokens
that are analyzed as some kind of verb form.
"""
...
# ╔═╡ 1c12ee0a-194d-4c06-85b0-d359b97aae32
md"""> **Drill down**"""

# ╔═╡ c4c946a9-e9cc-4b7a-a7ba-ba338a5d435c
dummymenu = ["" => ""]

# ╔═╡ 9167e9a1-3604-4175-9bd9-52cdf7c1f772
md"""> **Metadata we need**"""
...
# ╔═╡ b8ba859c-3e20-42c9-8ce4-f21425070b46
"""Compose list of lexemestrings for an `AnalyzedToken`
"""
function lexemestrings(atkn)
	 map(atkn.analyses .|> lexemeurn) do a
...
verbhisto = begin
	lexemelist = verbtokens .|> lexemestrings |> Iterators.flatten |>  collect
	sort!(OrderedDict(countmap(lexemelist)); byvalue = true, rev = true)
end

# ╔═╡ 8448bd9b-4924-4bb3-ae55-0c899b5cc479
"""Create user-selectable menu of verbs in the selected corpus,
sorted by frequency."""
function verbsmenu()
	histmenu = Pair{String, String}["" => "--Choose a verb--"]
	for k in keys(verbhisto)
		count = verbhisto[k]
		id = split(k, "@")[1]
		display = count == 1 ? string(k, " (", count, " passage)")  : string(k, " (", count, " passages)")
		push!(histmenu, id => display)
...
# ╔═╡ 50a15ad6-900c-4fba-bb09-2338dbf79d81
tokenindex = isnothing(corpus) ? nothing : corpusindex(corpus, lg)

# ╔═╡ d1faafd6-e6cc-4378-95de-1cb3ee493dcd
lexindex =  isnothing(analyzedtokenlist) ? nothing :  lexemedictionary(analyzedtokenlist, tokenindex)

# ╔═╡ d8e4f055-ef65-4a73-8bc6-7c045409eba6
"""Create user-selectable menu of forms for the selected verb,
sorted by frequency."""
function formsmenu(v)
	histmenu = Pair{String, String}["" => "--Choose a form--"]


	
	for k in keys(lexindex[v])
		count = length(lexindex[v][k])
		push!(histmenu, k => string(k, " (", count, " passages)"))
	end
	histmenu
end
...
# ╔═╡ 1c485f40-bacf-4802-b19b-4001901cd807
"""Compose markdown display of selected passages"""
function displaypassages(vb, frm)
	psgs = ["""*Occurrences of **$(frm)**:*""", ""]
	for psg in lexindex[vb][frm]
		push!(psgs, formatpassage(psg, frm))
	end
	join(psgs,"\n") |> Markdown.parse
end

# ╔═╡ e5d44413-c3dd-46d8-8bdb-018632e113f6
isempty(verbform) ? md"" : displaypassages(verb, verbform)
...
julia_version = "1.9.1"
manifest_format = "2.0"
project_hash = "4a890fb2b076b6474d3a86ad9b1785c8f31fd089"
...
git-tree-sha1 = "a0bf7d824e2bf2bb8362a366a922ba4a871b7435"
...
git-tree-sha1 = "8eee047fc7268dfc5b29107435227faa97f9d860"
...
git-tree-sha1 = "a2b4d20df12bf2c593ec379a837222c2c8345a27"
