...
#annotations_url = "https://raw.githubusercontent.com/neelsmith/LatinAndLatinSyntax/main/data/Lysias1_annotations.cex"
...
open(joinpath(outputdir, "syntax.css"), "w") do io
    write(io, LatinSyntax.defaultcss() * "\n")
end
open(joinpath(outputdir, "page.css"), "w") do io
    write(io, LatinSyntax.pagecss())
end
...
function webpage(idx, sentences, groups, tokens)
    ...
    key1 = "<div class=\"key right\"><strong>Highlighting</strong>:" *  LatinSyntax.sovkey() * "</div>"
    ...
    pagegroups = LatinSyntax.groupsforsentence(sentence, groups)
    key2 = "<div class=\"key left\"><strong>Color code</strong>:" * LatinSyntax.htmlgrouplist(pagegroups) * "</div>"
...
#publishsentence(16, sentences, groups, tokens)  
